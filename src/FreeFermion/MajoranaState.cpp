...
void MajoranaState::particles_at(const Qubits& sites) {
...
MajoranaState::MajoranaState(uint32_t L, std::optional<Qubits> sites) : FreeFermionState(L) {
  if (sites) {
...
std::vector<double> MajoranaState::williamson_eigenvalues(const Eigen::MatrixXd& A) const {
...
Eigen::MatrixXd MajoranaState::prepare_hamiltonian(const FreeFermionHamiltonian& H) const {
...
void MajoranaState::evolve(const Eigen::MatrixXd& U) {
  M = U * M * U.adjoint();
}

void MajoranaState::evolve_hamiltonian(const FreeFermionHamiltonian& H, double t) {
  Eigen::MatrixXd U = (t * prepare_hamiltonian(H)).exp();
  evolve(U);
}

void MajoranaState::forced_majorana_measurement(size_t i, bool outcome) {
...
void MajoranaState::forced_projective_measurement(size_t i, bool outcome) {
  forced_majorana_measurement(2*i, outcome);
}

double MajoranaState::occupation(size_t i) const {
  return (1.0 - M(2*i, 2*i + 1)) / 2.0;
}

double MajoranaState::entanglement(const QubitSupport& support, uint32_t index) {
...
std::string MajoranaState::to_string() const {
  std::stringstream s;
  s << M;
  return s.str();
}

void ExtendedMajoranaState::evolve_hamiltonian(const FreeFermionHamiltonian& H, double t) {
  for (auto& [amplitude, state] : states) {
    state.evolve_hamiltonian(H, t);
  }
}

void ExtendedMajoranaState::forced_projective_measurement(size_t i, bool outcome) {
  for (auto& [amplitude, state] : states) {
    state.forced_projective_measurement(i, outcome);
  }
}

std::string ExtendedMajoranaState::to_string() const {
  std::stringstream s;
  for (auto& [amplitude, state] : states) {
...
double ExtendedMajoranaState::occupation(size_t i) const {
...
void ExtendedMajoranaState::interaction(size_t i) {
  if (i > L - 1) {
...
double ExtendedMajoranaState::entanglement(const QubitSupport& support, uint32_t index) {
